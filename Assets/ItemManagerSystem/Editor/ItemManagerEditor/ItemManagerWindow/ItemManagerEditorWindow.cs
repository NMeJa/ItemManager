...
namespace ItemManagerSystem.ItemManagerEditor
{
    public class ItemManagerEditorWindow : EditorWindow
    {
        ...
        [MenuItem("Tool/NMJ/Item Manager")]
        ...
        private void HeaderContainer()
        {
            ...
            EditorWaitForSeconds editorWaitForSeconds = new EditorWaitForSeconds(0.45f);

            SearchContainers.RegisterValueChangedCallback(evt =>
                {
                    if (coroutine != null) EditorCoroutineUtility.StopCoroutine(coroutine);
                    List<ItemContainer> newList = containers.Search(evt.newValue);
        ...
        private void SetContainerItems(List<ItemContainer> items)
        {
            SearchContainers.menu.MenuItems().Clear();
            foreach (ItemContainer element in items)
            {
        ...
        private void BodyContainer()
        {
            ...
            void ItemsContainer()
            {
                ...
                ItemDatas.bindItem = (e, i) =>
                    {
                        ItemContainerElement item = (ItemContainerElement) e;
                ...
                {
                    const string ContainerName_VE = "ContainerName_VE";
                    ...
                    root.Q<Button>(EditContainerName_Btn).clicked += () =>
                        {
                            if (selectedContainer is null)
                            {
                                EditorUtility.DisplayDialog("First Select Container", "You should Select Container",
                                                            "Ok");
                                return;
                            }

                            ShowEditMode(true);
                            editNameTextField.value = selectedContainer.name;
                        };
                    root.Q<Button>(SaveContainer_Btn).clicked += () =>
                        {
                            string newName = editNameTextField.value;
                            int i = 0;
                            string tmp = newName;

                            while (containers.Any(n => n.Name == tmp))
                                tmp = $"{newName} {i++}";
                            ...
                            ItemDatas.RefreshItems();
                            SetContainerItems(containers);
                            SearchContainers.Q<TextField>().SetValueWithoutNotify(newName);
                            Repaint();
                        };
                    root.Q<Button>(CancelContainer_Btn).clicked += () => ShowEditMode(false);

                    void ShowEditMode(bool isEditMode)
                    {
                        containerName.style.display = !isEditMode ? DisplayStyle.Flex : DisplayStyle.None;
                        editModeContainer.style.display = isEditMode ? DisplayStyle.Flex : DisplayStyle.None;
                    }
                }
            }
            ...
            void AttributesGroupBox(bool isFirst = true)
            {
                ...
                void SetValues(string attributeName, string variableName)
                {
                    ...
                    bool useAttributeValue = useAttribute.value;
